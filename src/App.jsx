  ...
  return (
    <div style={{ minHeight: '100vh' }}>
      <header style={{
        ...
        <div>
          <div style={{ fontFamily: 'var(--ff-display)', fontSize: 22, fontWeight: 700, letterSpacing: -0.3 }}>{APP_EMOJI} {APP_NAME}</div>
          <div style={{ fontSize: 10, color: 'var(--text-3)', letterSpacing: 1.8, textTransform: 'uppercase', marginTop: 1 }}>{APP_TAGLINE}</div>
        </div>
          ...
          <nav style={{ display: 'flex', gap: 3, background: 'var(--surface-2)', borderRadius: 'var(--r-pill)', padding: 3 }}>
            {TABS.map(t => (
              <button key={t.id} onClick={() => { setTab(t.id); setCerca('') }}
                style={{ padding: '7px 12px', borderRadius: 'var(--r-pill)', border: 'none', fontSize: 12, fontWeight: 700, background: tab === t.id ? 'var(--surface-s)' : 'transparent', color: tab === t.id ? 'var(--accent)' : 'var(--text-3)', boxShadow: tab === t.id ? 'var(--shadow-s)' : 'none', transition: 'all 0.18s', cursor: 'pointer', whiteSpace: 'nowrap' }}>{t.label}</button>
            ))}
          </nav>
          <button onClick={logout} style={{ background: 'var(--surface-2)', border: 'none', borderRadius: 10, padding: '8px 12px', fontSize: 12, color: 'var(--text-3)', cursor: 'pointer', fontWeight: 700 }}>Esci</button>
        </div>
      </header>
          ...
          <div style={{ display: 'flex', gap: 8, overflowX: 'auto', paddingBottom: 6, marginBottom: 20 }}>
            {collezioni.map(c => (
              <button key={c.id} onClick={() => { setActiveCol(c.id); setCerca('') }}
                style={{ flexShrink: 0, padding: '8px 16px', borderRadius: 'var(--r-pill)', border: activeCol === c.id ? 'none' : '1.5px solid var(--border-s)', background: activeCol === c.id ? c.colore : 'rgba(255,252,247,0.7)', color: activeCol === c.id ? '#fff' : 'var(--text-2)', fontWeight: 700, fontSize: 13, boxShadow: activeCol === c.id ? `0 4px 14px ${c.colore}55` : 'none', transition: 'all 0.18s' }}>{c.nome}</button>
            ))}
            <button onClick={() => { setTarget(null); setModal('nuovaCol') }}
              style={{ flexShrink: 0, padding: '8px 16px', borderRadius: 'var(--r-pill)', border: '1.5px dashed var(--border-s)', background: 'none', color: 'var(--text-3)', fontWeight: 700, fontSize: 13, cursor: 'pointer' }}>+ Collezione</button>
          </div>
